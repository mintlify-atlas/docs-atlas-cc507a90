...
Running Deploy Your App on your own infrastructure requires configuring environment variables across several independent services: the Next.js web app, the Express API server, the BullMQ builder, the worker, and the notification microservice. This page is the authoritative reference for every variable each service reads, with its type, default value, and purpose.
