...
| Framework | Default Build Command | Default Start Command |
|-----------|-----------------------|-----------------------|
| Next.js | `npm run build` (or `bun run build` / `pnpm run build` / `yarn build`) | `node server.js` (via `next start`) |
| Node.js | `npm run build --if-present` | `npm start` (or `bun run start` for bun projects) |
| Vite | `npm run build` (or package-manager variant) | N/A — output is extracted as static files |

For Node.js projects that have no `start` script but do have an `index.js` at the root, the builder automatically falls back to `node index.js` as the suggested start command.
...
Overrides are passed into Dockerfiles as build arguments. You can inspect the templates in `infra/docker/` to understand exactly how each argument is consumed.
...
| `START_COMMAND` | Overrides the `CMD` used to start the container | `node dist/server.js` |
| `ROOT_DIR` | Sets the working directory inside the Docker build context | `apps/api` |
