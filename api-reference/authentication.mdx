...
3. GitHub redirects back to `{BETTER_AUTH_URL}/api/auth/callback/github` with a temporary authorization code.
...
You can verify your current session at any time by calling the `better-auth` session endpoint, which is proxied through the Next.js route handler:
