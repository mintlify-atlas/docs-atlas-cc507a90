...
description: "Explore how Deploy Your App organizes its monorepo into microservices, shared packages, and infrastructure layers connected by BullMQ queues and Redis."
---
...
| Service | Path | Role |
|---------|------|------|
| web | `apps/web` | Next.js dashboard frontend |
...
flowchart TD
    ...
    Proxy <--> Builder
    API <--> Postgres
    API --> DeploymentQueue["deployment-queue"]
    DeploymentQueue --> Worker
    Worker --> ExecutionQueue["execution-queue"]
    ExecutionQueue --> Builder
    Builder <--> Redis
    Notification <--> Redis
    Caddy <--> Proxy
    Caddy <--> Web
