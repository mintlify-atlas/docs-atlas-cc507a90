---
title: "Manage Environment Variables for Your Deployments"
...
- **Values are redacted from build logs.** The builder scans all log output for known env var values and replaces them with `[REDACTED]` before the logs are stored or streamed to the dashboard. This prevents accidental secret exposure in build output.
- **Encrypted ciphertext is never sent to the browser.** The project API returns `Env` records including the `key` but the encrypted `value` is not included in dashboard responses.
